...
service_opts = [
    ...
    cfg.StrOpt('restore_ceph_user', default='cinder',
               help='The Ceph restore user to connect with. Default here is to '
                    'use the same user as for Cinder volumes. '
                    'If not using ceph this should be set to None.'),
...
class CephBackupDriver(BackupDriver):
    ...
    def restore(self, backup, volume_id, volume_file):
        ...
        if backup_des and backup_des.find('cross_az') >= 0:
            self._ceph_restore_conf = strutils.safe_encode(
                CONF.restore_ceph_conf)
            self._ceph_restore_pool = strutils.safe_encode(
                CONF.restore_ceph_pool)
            self._ceph_restore_user = strutils.safe_encode(
                CONF.restore_ceph_user)
        else:
            self._ceph_restore_conf = self._ceph_backup_conf
            self._ceph_restore_pool = self._ceph_backup_pool
            self._ceph_restore_user = self._ceph_backup_user
